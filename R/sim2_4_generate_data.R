#' Generate sim2_4 simulation data (normal distribution, mixed L1 and L2)
#'
#' This function generates a synthetic spatial multi-omics dataset for Simulation 2_4.
...
#' @examples
#' \dontrun{
#' sim_data <- generate_sim2_4_data(seed = 2011)
#' str(sim_data)
#' }

generate_sim2_4_data <- function(seed = NULL) {
  if (!is.null(seed))
    set.seed(seed)
  ...
  data_2$L2[which(data_2$L2 == 3)] <- sample(c(3, 4), length(which(data_2$L2 == 3)), replace = TRUE)
  random_idx_2 <- sample(1:nrow(data_2), size = 50, replace = FALSE)
  data_2$L2[random_idx_2] <- sample(c(3, 4), size = length(random_idx_2), replace = TRUE)
