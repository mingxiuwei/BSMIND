#' Generate Sim2_1 simulation data (normal distribution, special label structure)
#'
#' This function generates a synthetic spatial multi-omics dataset for Simulation 2_1.
#' L1 labels follow the original 5 domains, while L2 labels are mostly 1 with
#' a few points set to 5. Feature values are generated from normal
#' distributions with domain-specific means.
#'
...
#' @examples
#' \dontrun{
#' sim_data <- generate_sim2_1_data(seed = 2011)
#' str(sim_data)
#' }

generate_sim2_1_data <- function(seed = NULL) {
  if (!is.null(seed))
    set.seed(seed)
  ...
  for (i in 1:n_features) {
    ...
    for (j in 1:nrow(params_L1)) {
      ...
      new_col[idx] <- rnorm(sum(idx), params_L1$mean[j], params_L1$sd[j])
    }
  ...
  for (i in 1:n_features) {
    ...
    for (j in 1:nrow(params_L2)) {
      ...
      new_col[idx] <- rnorm(sum(idx), params_L2$mean[j], params_L2$sd[j])
    }
