...
#' @examples
#' \dontrun{
#' sim_data <- generate_sim1_1_data(seed = 2010)
#' str(sim_data)
#' }

generate_sim1_1_data <- function(seed = NULL) {
  if (!is.null(seed))
    set.seed(seed)
  
  # 1. Create coordinate grid (22x22)
  x <- rep(1:22, each = 22)
  y <- rep(1:22, times = 22)
  coord <- data.frame(x, y)
  
  # 2. Generate C_label (spatial domains)
  coord$C <- ifelse(
  ...
  # 3. Generate label_L1 (mixing 3 and 4)
  data_1 <- coord[, c("x", "y")]
  data_1$L1 <- ifelse(
  ...
  # 4. Generate label_L2 (mixing 1 and 5)
  data_2 <- coord[, c("x", "y")]
  data_2$L2 <- ifelse(
  ...
  # 5. Generate numeric features for each modality (20 features each)
  n_features <- 10
  ...
  for (i in 1:n_features) {
    ...
    data_2[[paste0("adt_", i)]] <- new_col
  }
  
  # 6. Prepare final output: remove auxiliary columns
  coord <- coord[, -3]
